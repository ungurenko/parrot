...
- Модели скачиваются один раз через приложение (кнопка «Подготовить») и хранятся локально; любую можно удалить из настроек.
- Parakeet дополнительно сам ставит MLX-ускоритель (~1 ГБ) — после этого распознавание идёт заметно быстрее, с откатом на CPU-версию при сбое.
- Режимы Qwen3-ASR доступны, если на машине установлен их CLI (см. `docs/qwen-mlx.md`); иначе показываются как недоступные.
...
- **MLX-подсистема:** Python venv в пользовательской папке — `mlx-lm` / `mlx-vlm` для конспекта, `mlx-qwen3-asr` для Qwen-режимов (dev).
