...
| Путь | Назначение |
|------|------------|
| `src-tauri/src/lib.rs` | Entry, IPC-команды (enqueue, cancel_job, download_model, summarize, cancel_summary), preload активного движка, cancel_all на закрытии окна |
...
| `src-tauri/src/settings.rs` | JSON-настройки (save_dir, engine, language, onboarded, summarizer_enabled) |
...
- **Прогресс саммарайзера** — собственный ticker в `lib.rs::summarize`: 0→95% по оценке `4s load + expected_tokens/60 tok/s`. Фазы `loading` → `generating` → `finalizing` (100% при завершении).
- **Прогресс подготовки модели** — `download_model` для Qwen / саммарайзера поллит размер кеш-директории относительно ожидаемых байт. События: `model:progress` + `model:stage` (для ASR) и `summary_model:progress` + `summary_model:stage` (для саммарайзера). Стадии `downloading` → `warmup` → `ready`.
- **Проверка полноты моделей** — `transcriber_qwen::model_cache_exists` и `summarizer_qwen3::model_cache_exists` сравнивают размер HF-кеша с ожидаемым (≥90%). Защищает от запуска при частично скачанной модели.
